...
class BpmnModeler extends Component {
  ...
  componentDidMount() {
    ...
    this.modelerInstance.importXML(diagramXML, err => {
      if (err) {
        ...
        this.setState({
          error: "Failed to initialize canvas layout view."
        });
      }
    });
  }
  ...
  executeAiMutation() {
    ...
    this.modelerInstance.saveXML({
      format: true
    }, (err, currentXml) => {
      if (err) {
        this.setState({
          isLoading: false,
          error: "Failed to extract current layout map."
        });
        return;
      }

      fetch(`${process.env.REACT_APP_API_BASE_URL}/api/bpmn/generate`, {
          method: "POST",
          headers: {
